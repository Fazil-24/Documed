...
# Function to parse the lab test data from the reports
def parse_lab_test_data(text):
    data = []
    lines = text.split('\n')
    for line in lines:
        parts = line.split('\t')
        if len(parts) == 4:
            parameter, result, units, reference = parts
            data.append({
                'Parameter': parameter.strip(),
                'Result': float(result.strip()),
                'Units': units.strip(),
                'Reference Interval': reference.strip()
            })
    return data

# Function to create lab test dataframe from parsed data
def create_lab_test_dataframe(parsed_data):
...
def display_dashboard(df):
    ...
    # Bar chart for lab test results
    fig = px.bar(df, x='Parameter', y='Result', text='Result', title="Lab Test Results")
    st.plotly_chart(fig)

    # Line chart for lab test results
    fig = px.line(df, x='Parameter', y='Result', text='Result', title="Lab Test Results Over Time")
    st.plotly_chart(fig)
...
action = st.sidebar.selectbox("Choose Action", ["Chatbot", "Summarize", "Dashboard", "Notes"])

# Initialize extracted_texts
...
if action == "Chatbot":
    ...
    if query:
        if vectordb:
            relevant_chunks = get_relevant_passage(query, vectordb, n_results=5)
    ...
    if vectordb and extracted_texts:
        extracted_sections = categorize_extracted_text(extracted_texts)
        
        summary_data = {
            "patient_details": {
                "summary": generate_section_summary("Patient Details", extracted_sections["patient_details"]),
                "references": extract_references(extracted_sections["patient_details"])
            },
            "medical_history": {
                "summary": generate_section_summary("Medical History", extracted_sections["medical_history"]),
                "references": extract_references(extracted_sections["medical_history"])
            },
            "ongoing_treatment": {
                "summary": generate_section_summary("Ongoing Treatment", extracted_sections["ongoing_treatment"]),
                "references": extract_references(extracted_sections["ongoing_treatment"])
            },
            "images": extracted_sections["images"]
        }
